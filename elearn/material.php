<?php include 'assets/includes/header.php';
        ...
        $sql = "SELECT m.title AS mtitle,m.link,m.id,m.type,c.title FROM material AS m JOIN course AS c ON m.course = c.id ";

  if (isset($_GET['delete'])) {
            $mid = $_GET['delete'];
            $delmat = $connect2db->prepare("DELETE FROM material WHERE id = ?");
            if ($delmat->execute([$mid])) {
                $e="Deleted"; 
                echo  " <script>alert('$e');window.location='uploaded-material';</script> ";
            }
        }
                    ...
                    <tbody>
                        ...
                        <tr class="">
                            ...
                            <td class="text-center">
                                <?php 
                                    if ($std->type==1) {?>
                                        <a target="_blank" href="<?php echo "lecturer/".$std->link?>" class="text-success">
